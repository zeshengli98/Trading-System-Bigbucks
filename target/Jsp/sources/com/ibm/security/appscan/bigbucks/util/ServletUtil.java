...
import com.ibm.security.appscan.bigbucks.model.Feedback;
import com.ibm.security.appscan.bigbucks.model.User;
...
public class ServletUtil {
		...
		try {
			...
			while ((line=propertyReader.readLine()) != null && counter < 99){
				...
				if (equalsIndex >= 0 && line.length()>=equalsIndex){
					...
					Log4Bigbucks.getInstance().logError(error);
			...
			Log4Bigbucks.getInstance().logError("Failed to initialize demo properties with error: "+e.getMessage());
			return;
		}
	}
	...
	public static void initializeLogFile(ServletContext servletContext) {
		...
		logFile = new File(Log4Bigbucks.getInstance().getLogFileLocation());
		...
		try {
			...
			Log4Bigbucks.getInstance().logError("Failed to initialize log file with error: "+e.getMessage());
			return;
		} finally {
			if (logReader != null)
	...
	static public void initializeRestAPI(ServletContext servletContext) {
		...
		try {
			...
			Log4Bigbucks.getInstance().logError("Can't automatically initialize Swagger proerties.\n "
